...
- (void)findBarcodesInFrame:(UIImage *)uiImage
  ...
  NSArray *jabcodes = findJabcodesInFrame(uiImage, CGPointMake(scaleX, scaleY));
  if (jabcodes.length > 0) {
    completed(jabcodes);
    return;
  }
...
- (NSArray *)processBarcodes:(NSArray *)barcodes {
  ...
  for (FIRVisionBarcode *barcode in barcodes) {
    ...
    case FIRVisionBarcodeValueTypeWiFi:
      if (barcode.wifi.ssid) {
        [resultDict setObject:barcode.wifi.ssid forKey:@"ssid"];
      }
      if (barcode.wifi.password) {
        [resultDict setObject:barcode.wifi.password forKey:@"password"];
      }
    ...
    case FIRVisionBarcodeValueTypeURL:
      if (barcode.URL.url) {
        [resultDict setObject:barcode.URL.url forKey:@"url"];
      }
      if (barcode.URL.title) {
        [resultDict setObject:barcode.URL.title forKey:@"title"];
      }
      break;
    case FIRVisionBarcodeValueTypeContactInfo:
      if (barcode.contactInfo.addresses) {
        NSMutableArray *addresses = [[NSMutableArray alloc] init];
        for (FIRVisionBarcodeAddress *address in barcode.contactInfo
                 .addresses) {
      ...
      if (barcode.contactInfo.urls) {
        [resultDict setObject:barcode.contactInfo.urls forKey:@"urls"];
      }
      if (barcode.contactInfo.organization) {
        [resultDict setObject:barcode.contactInfo.organization
                       forKey:@"organization"];
      }
      break;
    case FIRVisionBarcodeValueTypeSMS:
      if (barcode.sms.message) {
        [resultDict setObject:barcode.sms.message forKey:@"message"];
      }
      if (barcode.sms.phoneNumber) {
        [resultDict setObject:barcode.sms.phoneNumber forKey:@"phoneNumber"];
      }
      break;
    case FIRVisionBarcodeValueTypeGeographicCoordinates:
      if (barcode.geoPoint.latitude) {
        [resultDict setObject:@(barcode.geoPoint.latitude) forKey:@"latitude"];
      }
      if (barcode.geoPoint.longitude) {
        [resultDict setObject:@(barcode.geoPoint.longitude)
                       forKey:@"longitude"];
      }
      break;
    case FIRVisionBarcodeValueTypeDriversLicense:
      if (barcode.driverLicense.firstName) {
        [resultDict setObject:barcode.driverLicense.firstName
                       forKey:@"firstName"];
      }
      if (barcode.driverLicense.middleName) {
        [resultDict setObject:barcode.driverLicense.middleName
                       forKey:@"middleName"];
      }
      if (barcode.driverLicense.lastName) {
        [resultDict setObject:barcode.driverLicense.lastName
                       forKey:@"lastName"];
      }
      if (barcode.driverLicense.gender) {
        [resultDict setObject:barcode.driverLicense.gender forKey:@"gender"];
      }
      if (barcode.driverLicense.addressCity) {
        [resultDict setObject:barcode.driverLicense.addressCity
                       forKey:@"addressCity"];
      }
      if (barcode.driverLicense.addressState) {
        [resultDict setObject:barcode.driverLicense.addressState
                       forKey:@"addressState"];
      }
      if (barcode.driverLicense.addressStreet) {
        [resultDict setObject:barcode.driverLicense.addressStreet
                       forKey:@"addressStreet"];
      }
      if (barcode.driverLicense.addressZip) {
        [resultDict setObject:barcode.driverLicense.addressZip
                       forKey:@"addressZip"];
      }
      if (barcode.driverLicense.birthDate) {
        [resultDict setObject:barcode.driverLicense.birthDate
                       forKey:@"birthDate"];
      }
      if (barcode.driverLicense.documentType) {
        [resultDict setObject:barcode.driverLicense.documentType
                       forKey:@"documentType"];
      }
      if (barcode.driverLicense.licenseNumber) {
        [resultDict setObject:barcode.driverLicense.licenseNumber
                       forKey:@"licenseNumber"];
      }
      if (barcode.driverLicense.expiryDate) {
        [resultDict setObject:barcode.driverLicense.expiryDate
                       forKey:@"expiryDate"];
      }
      if (barcode.driverLicense.issuingDate) {
        [resultDict setObject:barcode.driverLicense.issuingDate
                       forKey:@"issuingDate"];
      }
      if (barcode.driverLicense.issuingCountry) {
        [resultDict setObject:barcode.driverLicense.issuingCountry
                       forKey:@"issuingCountry"];
      }
      break;
    case FIRVisionBarcodeValueTypeCalendarEvent:
      if (barcode.calendarEvent.eventDescription) {
        [resultDict setObject:barcode.calendarEvent.eventDescription
                       forKey:@"eventDescription"];
      }
      if (barcode.calendarEvent.location) {
        [resultDict setObject:barcode.calendarEvent.location
                       forKey:@"location"];
      }
      if (barcode.calendarEvent.organizer) {
        [resultDict setObject:barcode.calendarEvent.organizer
                       forKey:@"organizer"];
      }
      if (barcode.calendarEvent.status) {
        [resultDict setObject:barcode.calendarEvent.status forKey:@"status"];
      }
      if (barcode.calendarEvent.summary) {
        [resultDict setObject:barcode.calendarEvent.summary forKey:@"summary"];
      }
      if (barcode.calendarEvent.start) {
        [resultDict setObject:[self processDate:barcode.calendarEvent.start]
                       forKey:@"start"];
      }
      if (barcode.calendarEvent.end) {
        [resultDict setObject:[self processDate:barcode.calendarEvent.end]
                       forKey:@"end"];
      }
      break;
    case FIRVisionBarcodeValueTypePhone:
      if (barcode.phone.number) {
        [resultDict setObject:barcode.phone.number forKey:@"number"];
      }
      if (barcode.phone.type) {
        [resultDict setObject:[self getPhoneType:barcode.phone.type]
                       forKey:@"phoneType"];
      }
      break;
    case FIRVisionBarcodeValueTypeEmail:
      if (barcode.email.address) {
        [resultDict setObject:barcode.email.address forKey:@"address"];
      }
      if (barcode.email.body) {
        [resultDict setObject:barcode.email.body forKey:@"body"];
      }
      if (barcode.email.subject) {
        [resultDict setObject:barcode.email.subject forKey:@"subject"];
      }
      if (barcode.email.type) {
        [resultDict setObject:[self getEmailType:barcode.email.type]
                       forKey:@"emailType"];
      }
      break;
    default:
